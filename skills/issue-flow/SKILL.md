...
3. superpowers 可用性检查 — 检查 `~/.claude/plugins/superpowers` 或 `~/.codex/superpowers` 目录是否存在，若不可用则停止，提示：`issue-flow requires superpowers plugin. Install with: /plugin install superpowers@claude-plugins-official`
